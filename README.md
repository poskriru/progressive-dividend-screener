...
- 「株式指標」「累進配当候補」等の各シートで、日付列はシートの日付値、金額は3桁区切りなどの表示書式で出力
- 条件に合う銘柄を「累進配当候補」シートへランキング出力
- 発行済株式数の大幅変動を「株式分割等確認対象」へ出力
- 自動補正対象外の企業行動を「ライツイシュー等確認対象」へ出力
...
表示し、理由の内訳を件数付きで集計して通知します。候補に変更がない場合もその旨を通知します。
...
`ExRT=3`（ライツイシュー）や種別不明の非1係数が保存されている銘柄は、
同じWorkflow内で「ライツイシュー等確認対象」シートへ出力します。
権利落ち日、調整係数、出典、現在の補正状態を確認できます。ライツイシューは
...
毎週日曜日の9時に`analyze_dividend_history.py`が配当履歴の品質を
確認し、結果をDiscordへ通知します。年次財務・配当レコード数、
ゼロ配当・負の配当、同一決算期の重複、配当履歴年数分布、
5期累進配当候補の件数を確認できます。負の配当や重複が見つかった場合は
Discord通知が赤色になります。手動実行も可能です。
...
結果には配当利回り、調整後5期配当CAGR、最新期の調整後年間配当、配当性向、PER、PBR、ROEを表示します。Discordの文字数上限を超える場合は後続を省略します。
