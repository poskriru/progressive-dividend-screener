...
--
-- corporate_actionsに複数の取得元が保存されても、
-- 同一の企業行動を重複して調整係数へ適用しないようにする。
--
-- JPXの完全性判定をadjusted指標へ接続するまでは、
-- 既存のJ-Quants取得範囲とJ-Quants企業行動だけを使用する。
-- ============================================================

CREATE OR REPLACE VIEW screener.company_dividend_metrics_adjusted AS
...
coverage AS (
    ...
    LEFT JOIN screener.corporate_actions AS actions
       ...
       AND actions.source = 'J-Quants V2'
    GROUP BY
        raw_metrics.security_code,
        raw_metrics.oldest_fiscal_period_end_5y,
