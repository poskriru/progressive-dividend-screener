-- ============================================================
-- 014_use_jpx_current_rights_watchlist.sql
--
-- JPX公式月次PDFが前月末まで連続して取得済みであり、
-- 最新株価月のJPX当月権利処理ページが正常取得済みの場合、
-- 当月watchlistに掲載されていない銘柄だけを
-- 暫定的に補正範囲完全として扱う。
--
-- 当月watchlist掲載銘柄は、月次PDFで正式な権利落ち日と
-- adjustment_factorが確定するまで補正範囲不完全とする。
--
-- JPX月次PDFが最新株価日まで到達している場合は、
-- 当月watchlistの有無にかかわらず月次PDFを優先する。
--
-- watchlistのallocation_dateは割当日であり、
-- effective_dateや権利落ち日として使用しない。
--
...
current_rights_snapshot AS (
    SELECT
        snapshots.coverage_month,
        snapshots.fetched_at,
        snapshots.source_count,
        snapshots.record_count,
        ...
        (
            ...
            FROM
                screener.jpx_current_rights_watchlist
                    AS stored_watchlist
            WHERE stored_watchlist.coverage_month
                = snapshots.coverage_month
    ...
    WHERE latest_market_data.latest_trading_date
            IS NOT NULL
      AND snapshots.coverage_month
            = DATE_TRUNC(
                'month',
...
-- 銘柄ごとの補正範囲と未対応企業行動
--
-- 次のどちらかを満たす場合だけrange_is_complete=true。
--
-- 1. 月次PDFが最新株価日まで到達している
--
-- 2. 月次PDFが最新株価月の前月末まで到達しており、
--    最新株価月の当月ページが正常取得済みで、
--    当該銘柄がwatchlistに掲載されていない
-- ============================================================
...
coverage AS (
            ...
            WHEN
                ...
                AND (
                    ...
                    (
                        ...
                        AND current_snapshot.coverage_month
                            = DATE_TRUNC(
                                'month',
                                latest_market_data
                                    .latest_trading_date
                        ...
                        AND current_snapshot.source_count > 0

                        -- ====================================
                        -- snapshot件数と保存件数が一致
                        -- ====================================

                        AND current_snapshot.record_count
                            = current_snapshot
                                .stored_record_count

                        -- ====================================
                        -- snapshotが最新株価日以降に取得済み
                        -- ====================================

                        AND (
                            current_snapshot.fetched_at
    ...
    GROUP BY
        ...
        jpx_status.months_are_continuous,
        latest_market_data.latest_trading_date,
        current_snapshot.coverage_month,
        current_snapshot.fetched_at,
        current_snapshot.source_count,
        current_snapshot.record_count,
        current_snapshot.stored_record_count
),
